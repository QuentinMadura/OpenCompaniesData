  ...
  getImgUrl: (state, getters, rootState) => (obj) => {
    ...
    if (!image) {
      ...
      if (images_set && images_set.length > 0) {
        ...
        image = `/static/illustrations/textures/medium_fiche_${
          (parseInt(id.substr(id.length - 6), 16) % textureCount) + 1
        }.png`;
      }
    }
    return image;
...
export const mutations = {
  ...
  setSearchQuestion(state, localEndpointConfig) {
    ...
    let authorizedDefaultArgs = ["page", "perPage", "shuffleSeed", "sortBy", "sortIsDescending"];
  ...
  search({ state, commit, dispatch, getters, rootGetters }) {
    ...
    searchPendingAbort.promise
      ...
      .catch((error) => {
        ...
        if (error.name !== "AbortError") commit("setSearchError", { error });
      });
  },
