...
export function createAllPoints(sourceId, vars, layerId = "all-points") {
  var layerConfig = {
    ...
    filter: ["==", "$type", "Point"],
    paint: {
      "circle-stroke-width": ["interpolate", ["linear"], ["zoom"], 9, 0, vars.max_zoom, 1],
      "circle-stroke-color": vars.circle_stroke_color,
      "circle-color": vars.circle_color,
...
export function createHeatmapLayer(sourceId, vars, layerId = "heatmap-layer") {
  ...
  let layerConfig = {
    ...
    paint: {
      ...
      "heatmap-weight": ["interpolate", ["linear"], ["get", vars.prop_weight], 0, 0, 0.1, 1],

      // Increase the heatmap color weight weight by zoom level
      // heatmap-intensity is a multiplier on top of heatmap-weight
      "heatmap-intensity": ["interpolate", ["linear"], ["zoom"], 0, 0.1, vars.max_zoom, 1.5],
      ...
      "heatmap-radius": ["interpolate", ["linear"], ["zoom"], vars.radius_min, vars.radius_max, vars.max_zoom, 100],

      // Transition from heatmap to circle layer by zoom level
      "heatmap-opacity": ["interpolate", ["linear"], ["zoom"], 6, 0.7, vars.max_zoom, 0.1],
    },
    layout: {
      visibility: vars.is_default_visible ? "visible" : "none",
    },
