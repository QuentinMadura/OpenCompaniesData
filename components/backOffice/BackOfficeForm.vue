  ...
  <div>
    <div class="card">
      ...
      <div v-show="isOpen" class="card-content">
        <div class="content">
          <!-- DEBUGGING -->
          rootUrlBackend : <code>{{ rootUrlBackend }}</code
          ><br />
          <!-- jwt : <code>{{ jwt }}</code><br> -->
          jwt.access_token : <code>{{ jwt.access_token }}</code
          ><br />
          <!-- jwt['access_token'] : <code>{{ jwt['access_token'] }}</code><br> -->

          configCollection : <code>{{ configCollection }}</code
          ><br />
          ...
          <form v-on:submit.prevent="sendConfigModif" name="formConfig">
            <!-- LOOP EDIT FIELDS LIST -->
            <div v-for="(editConf, index) in docConfig.edit" :key="index">
              <!-- DEBUGGING -->
              <!-- editConf : <code>{{ editConf }}</code><br> -->
              editConf in docConfig.edit: <br />
              <pre><code>{{ JSON.stringify(editConf, null, 1) }}</code></pre>
              <br />
              configToEdit(editConf.subfield) : <br />
              <pre><code>{{ JSON.stringify( configToEdit(editConf.subfield), null, 1) }}</code></pre>
              <br />

              <!-- CREATE FIELDS TO EDIT IF ANY -->
              <div v-for="conf in getDocSubfieldsToEdit(editConf)" :key="conf.subfield">
                <!-- text field -->
                <div v-if="conf.type === 'text'" class="field">
                  <label class="label">{{ conf.subfield }}</label>
                  <div class="control">
                    <input :value="configBloc[conf.subfield]" class="input" type="text" placeholder="Text input" />
                  </div>
                  <p class="help">
                    {{ configBloc["help"] }}
                  </p>
                ...
                <!-- select field -->
                <div v-if="conf.type === 'select'" class="field">
                ...
                <!-- textarea field -->
                <div v-if="conf.type === 'textarea'" class="field">
                ...
                <!-- check field -->
                <div v-if="conf.type === 'check'" class="field">
                  <div class="control">
                ...
                <!-- radio field -->
                <div v-if="conf.type === 'bool'" class="field">
                  <div class="control">
                    <label class="radio">
            ...
            <div class="field">
              <div class="control">
                <label class="checkbox">
                  <input type="checkbox" :value="isDefault" :checked="isDefault" />
...
export default {
  ...
  props: ["configCollection", "currentTab", "docConfig", "config"],
  ...
  methods: {
    ...
    sendConfigModif(e) {
      ...
      axios
        ...
        .then((response) =>
          this.$store.dispatch("config/getConfigType", {
            type: currentColl,
