<template>
  ...
  <section class="main-content skip-navbar has-background-white-ter columns is-fullheight no-margin-bottom">
    <!-- SIDE MENU -->
    <aside class="column noPaddingBottom is-full-mobile is-4-tablet is-3-desktop is-3-widescreen is-2-fullhd section">
      <p class="menu-label">
      ...
      <!-- MENUS -->
      <ul class="menu-list">
        <li v-for="menu in backOfficeMenu" :key="menu.config_coll">
          <nuxt-link
    ...
    <div class="container column noMarginLeft is-full-mobile is-7-tablet is-8-desktop is-8-widescreen is-8-fullhd">
      <div class="section">
        <!-- TABS -->
        <div class="tabs is-centered">
          <ul>
            ...
            >
              <a @click="pushTabRoute(tab.tab_code)">
                <!-- {{ tab.title }} -->
        ...
        <!-- DOCUMENTATION modal -->
        <br />
        <BackOfficeDocModal :currentColl="activeMenu" :currentTab="getTabConfig" />
        <!-- :currentTab="activeTab" -->
        <br /><br />

        <!-- JSON CONTENTS -->
        <div v-for="(docConfig, indexTabDoc) in tabDocs" :key="indexTabDoc">
          <!-- JSON data -->
          <BackOfficeDispatch
