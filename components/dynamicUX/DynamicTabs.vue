<template>
  <div class="container">
    <div
      v-if="tabConfig"
      :class="`tabs ${tabConfig.ui_options.size.value} ${tabConfig.ui_options.position.value} ${
        skipNavbar ? 'skip-navbar' : ''
      }`"
    ...
    >
      ...
      <ul :class="`cis-tabs apiviz-tabs`">
        <li
          v-for="tab in tabConfig.tabs_options"
          :key="tab.tab_code"
          :class="`${tab.link_to === currentRoutePath ? 'is-active' : ''}`"
        >
          <nuxt-link :to="tab.link_to" class="" @click="setActiveTab(tab.tab_code)">
