<template>
  <section class="search-results-stats">
    ...
    <div
      ...
      :style="`margin-right:${breakpoint.marginContainer}; margin-left:${breakpoint.marginContainer}`"
    >
      <SearchResultsCountAndTabs :view="VIEW_STAT" />

      <!-- MAIN CHART COMPONENT -->
      <div v-if="stats" class="columns is-multiline is-8">
        <div
        ...
        >
          ...
          <ApexChartComponent v-if="chart.is_activated" :chart="chart" :rawSerie="getCurrentRawSerie(chart.serie_id)">
          </ApexChartComponent>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    chartsList() {
      this.log &&
        console.log(
          "\nC-SearchResultsStats-getCurrentRawSerie / this.routeConfig.charts_list : ",
          this.routeConfig.charts_list
        );
  ...
  methods: {
    ...
    buildChartSeries(chartSettings) {
      ...
      this.log &&
        console.log(
          "C-SearchResultsStats-buildChartSeries / (before list only) dataObjectRefined :",
          dataObjectRefined
        );
