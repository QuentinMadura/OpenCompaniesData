<template>
  <li v-bind:class="{ item: hasChildren }">
    <div v-bind:style="{ color: '#' + getColor }" :class="{ bold: hasChildren }" @click="toggle">
      {{ getText }}
      <span v-if="hasChildren">[{{ isOpen ? "-" : "+" }}]</span>
    </div>
    <ul v-show="isOpen" v-if="hasChildren">
      <TreeItem v-for="(child, index) in item.children" :key="index" :item="child"></TreeItem>
    </ul>
  </li>
</template>
    ...
    getText: function () {
      ...
      if (this.item.data.computedValue) {
        text +=
          " erreur calculée :" +
          formatter.format(
            this.item.data.value ? this.item.data.value - this.item.data.computedValue : this.item.data.computedValue
          );
      }
