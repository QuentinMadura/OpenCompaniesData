<template>
  <section class="search-results-list">
    ...
    >
      <SearchResultsCountAndTabs :view="VIEW_TABLE" />

      <PaginationNav
        v-if="
          routePagination &&
          routePagination.is_visible &&
          ['top', 'top_and_bottom', 'both'].includes(routePagination.position)
        "
        :position="'top'"
      ...
      <div v-if="!pending && total > 0" class="table-container">
        <table class="table is-centered">
          <!-- HEADER -->
          <thead>
            <tr class="is-centered">
              <th class="is-size-7">détail</th>
              <th class="has-text-centered is-size-7">Name</th>
              <th class="has-text-centered is-size-7">Secteur (code APE)</th>
              <th class="has-text-centered is-size-7">CompanyNumber</th>
            </tr>
          </thead>
          ...
          <tbody>
            <tr v-for="(item, index) in projects" :key="index">
              <td class="has-text-centered">
                <nuxt-link :to="`/${dataset_uri}/detail?id=${item.siren.value}`">
                  &nbsp;
      ...
      <PaginationNav
        v-if="
          routePagination &&
          routePagination.is_visible &&
          ['bottom', 'top_and_bottom', 'both'].includes(routePagination.position)
        "
        :position="'bottom'"
        :feedback="routePagination.feedback"
      ...
      <div class="no-result error" v-if="!pending && total === 0">
        ...
        <div class="has-text-centered">
          ...
          <button
            ...
            href="/"
            class="button is-primary is-primary-b is-outlined"
