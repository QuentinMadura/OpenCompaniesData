<template>
  <div class="columns">
    ...
    >
      <div v-for="(textField, index) in textContentsFields" key="index">
        <h4 v-if="chart.chart_texts[textField] && textField === 'text_title'" class="has-text-weight-semibold">
          {{ translate(chart.chart_texts, textField) }}
        </h4>

        <p v-if="chart.chart_texts[textField] && textField !== 'text_title'" class="is-size-7 chart-text">
          {{ translate(chart.chart_texts, textField) }}
        </p>
      </div>
    </div>
    ...
    <div v-if="stats" :class="`apexChartComponent-frame column is-${innerColumnsSizes.chartCols}`">
      ...
      <!-- <div
        ...
        > -->
      <apexchart :type="chart.chart_type" :height="chart.height" :options="refinedOptions" :series="refinedSeries">
...
export default {
  ...
  data() {
    return {
      ...
      textContentsFields: ["text_title", "text_content_a", "text_content_b", "text_content_c"],
    };
  },
    ...
    buildChartSeries() {
      ...
      if (serieChartOptions && serieChartOptions.length > 0) {
        for (let option of serieChartOptions) {
          ...
          this.log &&
            console.log("C-ApexChartComponent-buildChartSeries / optionsBeforeList / optionPath :", optionPath);

          let buildFromPath = option.build_list_from.split("/");
          this.log &&
            console.log("C-ApexChartComponent-buildChartSeries / optionsBeforeList / buildFromPath :", buildFromPath);
      ...
      let dataObjectRefined = dataObject.map((item) => {
        ...
        this.log &&
          console.log(
            "C-ApexChartComponent-buildChartSeries / map / (after add_missing_values) serieData :",
            serieData
          );
        ...
        if (dataSerieSettings.need_sorting) {
          ...
          this.log &&
            console.log("C-ApexChartComponent-buildChartSeries / map / sorting / sortingField :", sortingField);
          serieData = serieData.sort((a, b) => (a[sortingField] > b[sortingField] ? 1 : -1));
          needLabelsRemap = true;
        }
        this.log &&
          console.log("C-ApexChartComponent-buildChartSeries / map / (after need_sorting) serieData :", serieData);
        ...
        this.log &&
          console.log("C-ApexChartComponent-buildChartSeries / map / (after need_remap) serieData :", serieData);
      ...
      this.log &&
        console.log(
          "C-ApexChartComponent-buildChartSeries / (before need_list_only) dataObjectRefined :",
          dataObjectRefined
        );
