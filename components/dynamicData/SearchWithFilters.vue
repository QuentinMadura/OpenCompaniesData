<template>
  <div
    :class="`search-bar ${shrinkNav ? 'search-bar-shrink' : ''} ${
      onlyIframe ? 'iframing' : ''
    } navbar is-white is-fixed-top has-bottom-border`"
    role="menubar"
    aria-label="filters navigation"
  >
    <div class="container">
      <!-- INPUT TEXT -->
      <div class="navbar-start custom-start">
        ...
        <div class="columns is-gapless is-mobile custom-input">
          <div :class="`column is-${showFiltersSwitch ? '10' : '11'}`">
            <div class="navbar-item is-paddingless">
              <div class="field is-large has-addons field-centered">
                ...
                <p class="control" v-show="textQuery !== ''">
                  <a class="button is-large is-right has-text-grey-light" @click="clearQuery">
                    <span class="icon">
          ...
          <div v-if="showFiltersSwitch" :class="`column is-2 is-centered`">
            <div class="navbar-item navbar-item-filter has-text-centered">
              <a :class="`button ${showFilters ? 'is-primary is-primary-c' : 'is-white'}`" @click="SwitchFilters()">
                <span :class="`icon ${showFilters ? '' : 'has-text-primary has-text-primary-c'}`">
      ...
      <!-- INPUT FILTERS -->
      <hr v-if="showFiltersSwitch && showFilters" class="is-flex-touch filters-delimiter" />
      <div v-if="showFilters" class="navbar-end has-background-white">
        <!-- is-hidden-touch (to completely hide from mobile)-->
        <!-- LOOP FILTERS LISTS -->
        <span
        ...
        >
          ...
          <a
            :class="`navbar-link is-arrrowless ${
              selectedFilters.get(filter.name).size >= 1 ? 'has-text-weight-semibold' : ''
            } `"
          ...
          <!-- CHOICES CONTAINER -->
          <div :id="filter.name" :ref="filter.name" class="navbar-dropdown is-right hide-choices no-padding-bottom">
            <!-- here make it collapsable -->

            <!-- LOOP CHOICES -->
            <a v-for="choice in filter.choices" :key="choice.name" class="navbar-item no-border">
              <div class="field is-narrow">
                <input
                  class="is-checkradio is-checkradio-c is-default is-normal"
                  :id="choice.name"
                  type="checkbox"
            ...
            <div class="reset-btn">
              <button
                class="button is-text no-underline is-fullwidth has-text-grey"
                :data-filter="filter.name"
...
export default {
  ...
  methods: {
    ...
    changeFilter({ target }) {
      this.$store.dispatch("search/toggleFilter", {
