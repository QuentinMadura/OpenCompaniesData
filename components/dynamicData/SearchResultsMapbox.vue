...
<template>
  ...
  <div class="map">
    ...
    <div class="count-and-tabs-container">
      ...
      >
        <SearchResultsCountAndTabs :view="VIEW_MAP" :open="showCard">
          <!-- HIGHLIGHTED ITEM  -->
          <div class="highlighted-project" v-if="showCard" slot="item">
            <!-- BUTTON TO CLOSE PREVIEW -->
            <button v-show="displayedProject" class="button close" @click="showCard = false">
            ...
            <div class="card">
              <!-- LOADER -->
              <div class="columns is-mobile is-vcentered" v-show="!displayedProject">
    ...
    <!-- {{ itemsForsMap }} -->
    <!-- LOADER -->
    <div id="loader-map" v-show="!itemsForMap || showLoader" class="lds-roller floating">
      <div></div>
      <div></div>
      <div></div>
      <div></div>
    ...
    <!-- LAYERS & LEGEND -->
    <div v-if="itemsForMap" id="legend" :class="`legend-block legend-bottom-right`">
      <!-- LAYERS SWITCH -->
      <div v-if="layersVisibility && layersVisibility.is_activated" class="legend layer-switch">
        <button class="button is-small is-fullwidth is-info-b is-outlined" @click="switchLayersDrawer()">
          {{ basicDict.map_layers[locale] }}
        </button>

        <div v-show="drawerLayersOpen" class="legend-content">
          <div v-for="(layer, index) in layersVisibility.layers_switches" :key="index" class="field">
            <input
              class="is-checkradio"
              :id="layer.label"
              :name="layer.label"
      ...
      <!-- SCALE LEGEND -->
      <div v-if="findCurrentChorosource" class="legend">
        <button class="button is-small is-fullwidth is-info-b is-outlined" @click="switchLegendDrawer()">
        ...
        <div v-show="drawerScalesOpen" class="legend-content">
          <div v-for="(scale, index) in findCurrentChorosource.legend.scales" :key="index">
            <div>
              <span :style="`background-color: ${scale.color}`"></span>
              {{ scale.value }}
            </div>
          </div>
        </div>
...
import { geoJsonBasesUrls, createGeoJsonDataPoints, updateGeoJsonProperties } from "~/plugins/geoJson.js";

import { getJson } from "~/plugins/utils.js";
...
export default {
  ...
  watch: {
    map(next, prev) {
      ...
      if (next && !prev) {
        ...
        if (!this.isClusterSet && this.itemsForMap) {
          this.log &&
            console.log("C-SearchResultsMapbox / watch - map - createGeoJsonDataPoints ( from geoJson.js ) ...");
          ...
          this.log &&
            console.log(
              "C-SearchResultsMapbox / watch - map - this.itemsForMap empty ... just wait for projects to change"
            );
        }
      }
    },
    ...
    getCorrespondingChoroConfigs(next, prev) {
      ...
      if (stringNextSourceIds !== stringPrevSourceIds) {
        ...
        this.log &&
          console.log(
            "\nC-SearchResultsMapbox / watch - getCorrespondingChoroConfigs / nextSourceIds : ",
            nextSourceIds
          );
        ...
        if (layersToCheck.length > 0 && nextSourceIds.length > 1) {
          this.log &&
            console.log(
              "C-SearchResultsMapbox / watch - getCorrespondingChoroConfigs / layersToCheck : ",
              layersToCheck
            );
          layersToCheck.forEach((sourceConfig) => {
            this.updateChoroSourceByZoom(sourceConfig);
  ...
  computed: {
    ...
    getCorrespondingChoroConfigs() {
      ...
      const mapboxChoroConfigs =
        mapboxOptions && mapboxOptions.choropleth_layer && mapboxOptions.choropleth_layer.sources;
      ...
      let choroConfigs =
        mapboxChoroConfigs &&
        mapboxChoroConfigs.filter((conf) => {
  ...
  methods: {
    ...
    joinProjectsToPolygon(source, dataLoaded, choroRefIdex, noDataProxy = false) {
      ...
      dataFeatures.forEach((i) => {
        const result = items.reduce(
          (sum, item) =>
            String(item[source.join_polygon_id_to_field]) === String(i.properties[source.polygon_prop_id])
              ? sum + 1
              : sum,
          0
        );
    ...
    createMapItems(geoJson) {
      ...
      let allPointsSourceId =
        mapboxOptions.all_points_layer && mapboxOptions.all_points_layer.source_id
          ? mapboxOptions.all_points_layer.source_id
          : "allPointsSource";
      let geoJsonSourceId =
        mapboxOptions.cluster_circles_layer && mapboxOptions.cluster_circles_layer.source_id
          ? mapboxOptions.cluster_circles_layer.source_id
          : "clusterSource";
    ...
    createAddGeoJsonSource(geoJson) {
      ...
      if (mapboxOptions.cluster_circles_layer && mapboxOptions.cluster_circles_layer.is_activated) {
        ...
        let geoJsonSourceId = clusterLayerConfigOptions.source_id
          ? clusterLayerConfigOptions.source_id
          : "clusterSource";
        ...
        for (let source of choroplethConfigOptions.sources) {
          if (source.is_activated) {
            ...
            if (source.need_aggregation && !isUpdate) {
              ...
              this.log &&
                console.log("C-SearchResultsMapbox / createChoroplethSource / source.need_aggregation && !isUpdate ");
            ...
            if (source.need_aggregation && !source.update_src_from_previous_source) {
              ...
              this.log &&
                console.log(
                  "C-SearchResultsMapbox / createChoroplethSource / source.need_aggregation + isUpdate : ",
                  isUpdate
                );
    ...
    updateChoroSourceByZoom(choroSourceConfig, featuresArray = undefined) {
      ...
      for (let update of choroSourceConfig.update_src_options) {
        ...
        if (!featuresArray) {
          ...
          this.log &&
            console.log("C-SearchResultsMapbox / updateChoroSourceByZoom / renderredCenters : ", renderredCenters);
          ...
          if (update.upper_load_feat === "only_center") {
            ...
            let nearestFeature = upperRenderredFeatures.find(
              (f) =>
                f.properties[update.upper_main_matching_prop] === nearest.properties[update.upper_main_matching_prop]
            );
        ...
        this.log &&
          console.log(
            "C-SearchResultsMapbox / updateChoroSourceByZoom / upperRenderredFeatures : ",
            upperRenderredFeatures
          );
    ...
    createAddGeoJsonLayers(geoJsonSourceId) {
      ...
      if (mapboxOptions.heatmap_layer && mapboxOptions.heatmap_layer.is_activated) {
        ...
        let heatmapSourceId = heatmapLayerConfigOptions.source_id
          ? heatmapLayerConfigOptions.source_id
        ...
        let heatmapLayerConfig = createHeatmapLayer(heatmapSourceId, heatmapLayerConfigOptions, heatmapLayerId);
      ...
      if (mapboxOptions.all_points_layer && mapboxOptions.all_points_layer.is_activated) {
        ...
        let allPointsConfig = createAllPoints(geoJsonSourceId.allPointsId, allPointsConfigOptions, allPointsLayerId);
        ...
        if (allPointsConfigOptions.is_clickable) {
          mapboxMap.on("click", allPointsLayerId, function (e) {
            ...
            console.log(
              "C-SearchResultsMapbox / createGeoJsonLayers / clic - all-points - featuresPoint : ",
              featuresPoint
            );
            ...
            console.log(
              "C-SearchResultsMapbox / createGeoJsonLayers / clic - all-points - coordinates : ",
              coordinates
            );
      ...
      if (mapboxOptions.cluster_circles_layer && mapboxOptions.cluster_circles_layer.is_activated) {
        ...
        let clusterSourceId = clusterLayerConfigOptions.source_id
          ? clusterLayerConfigOptions.source_id
          : "clusterSource";
        let clusterLayerId = clusterLayerConfigOptions.layer_id
          ? clusterLayerConfigOptions.layer_id
          : "cluster-circles";
      ...
      if (mapboxOptions.cluster_count_layer && mapboxOptions.cluster_count_layer.is_activated) {
        ...
        if (countLayerConfigOptions.is_clickable) {
          mapboxMap.on("click", countLayerId, function (e) {
            ...
            console.log(
              "C-SearchResultsMapbox / createGeoJsonLayers / clic - cluster-count -  featuresCluster : ",
              featuresCluster
            );
      ...
      if (mapboxOptions.cluster_unclustered_layer && mapboxOptions.cluster_unclustered_layer.is_activated) {
        ...
        let unclusteredLayerId = unclusteredLayerConfigOptions.layer_id
          ? unclusteredLayerConfigOptions.layer_id
          : "unclustered-points";
        ...
        if (unclusteredLayerConfigOptions.is_clickable) {
          mapboxMap.on("click", unclusteredLayerId, function (e) {
            ...
            console.log(
              "C-SearchResultsMapbox / createGeoJsonLayers / clic - unclustered-point - featuresPoint : ",
              featuresPoint
            );
            ...
            var coordinates = e.features[0].geometry.coordinates.slice();
            console.log(
              "C-SearchResultsMapbox / createGeoJsonLayers / clic - unclustered-point - coordinates : ",
              coordinates
            );
    ...
    createAddChoroplethLayers(source) {
      ...
      let choroplethConfig = createChoroplethLayer(choroplethSourceId, source, choroplethLayerId);
    ...
    getIconSize(item, highlightedItem) {
      if (highlightedItem) {
        return this.itemId(item, "block_id") === this.itemId(highlightedItem, "block_id")
          ? this.iconSizeHighlighted
          : this.iconSizeNormal;
      } else {
